...
The registry (``scenarios/__init__.py``) discovers scenarios by globbing
...
def emit_executive_business_metrics_if_eligible(service: Any) -> None:
    ...
    if not isinstance(scenario, YamlScenario):
        return
    ...
    for kpi in scenario._kpi_emissions:
        val = resolve(kpi["value"], rng)
        service.emit_metric(kpi["name"], float(val), kpi.get("unit", ""))
