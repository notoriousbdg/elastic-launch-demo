...
def build_slides(scenario, channels: dict, services: dict) -> list:
    ...
    slides.append({
        ...
        "icon":        icon,
        "name":        sname,
        "subtitle":    f"{n_ch} Chaos Channels — Demo Guide",
        "description": desc,
        "meta": {
            "Channels":  f"{n_ch} Fault Channels",
    ...
    # ── 19: Section — Auto-remediate ──────────────────────────────────────────
    slides.append({
        "type":  "section",
        "part":  "Part 2",
        "title": "Auto-Remediate (16–20)",
        "desc":  "Real faults that remediate without operator approval",
        "color": "#3fb950",
    })
...
function renderChannel(s) {
  ...
  const pillTxt = s.is_fault ? "⚠ Fault · Agent Required" : "✓ Auto-Remediate";
